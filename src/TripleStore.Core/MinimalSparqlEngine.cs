...
public sealed class MinimalSparqlEngine
{
    ...
    public IEnumerable<Dictionary<string, string>> ExecuteQuery(string sparql)
    {
        ...
        var rootPatterns = root.TriplePatterns.OfType<TriplePattern>().Select(ToSPO).ToList();
        ...
        foreach (var gp in root.ChildGraphPatterns)
        {
            if (!gp.IsGraph)
            {
                ...
                var pats = gp.TriplePatterns.OfType<TriplePattern>().Select(ToSPO).ToList();
            ...
            var graphPatterns = gp.TriplePatterns.OfType<TriplePattern>().Select(ToSPO).ToList();
